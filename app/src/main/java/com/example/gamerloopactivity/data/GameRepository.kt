...
import com.example.gamerloopactivity.domain.toSettingsMappers
...
class GameRepository @Inject constructor(
    ...
    suspend fun getGameSettings(gameId: Int): SettingsMappers? {
        Log.d(TAG, "Llamando a la API para detalles del juego ID: $gameId")
        return try {
            val response = apiService.getGameSettings(gameId)
            if (response.isSuccessful) {
                val body = response.body()
                if (body != null) {
                    Log.d(TAG, "Detalles del juego ID: $gameId recibidos y mapeados correctamente.")
                    body.toSettingsMappers()
                } else {
                    Log.e(
                        TAG,
                        "Error 200/204: La API de detalles devolvió un cuerpo nulo para el ID: $gameId"
